...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['delete_user_ids'])) {
        ...
        $success_message = "Solicitudes de eliminación enviadas correctamente.";
    }
...
<head>
    ...
    <script>
        ...
        function confirmDeletion() {
            return confirm("¿Estás seguro de que quieres enviar las solicitudes de eliminación para los usuarios seleccionados?");
        }

        function confirmUpdate() {
            return confirm("¿Estás seguro de que quieres enviar la solicitud de actualización?");
        }

        function toggleAll(source) {
            checkboxes = document.querySelectorAll('input[name="delete_user_ids[]"]');
...
<body>
    ...
    <form action="manage_users.php" method="POST" onsubmit="return confirmUpdate();">
        <table>
            <thead>
                <tr>
                    <th><input type="checkbox" onclick="toggleAll(this)" class="delete-checkbox" style="display:none;"></th>
                    <th>ID Usuario</th>
                    <th>Nombre</th>
                    <th>Apellido</th>
                    <th>Correo</th>
                    <th>Dirección</th>
                    <th>Rol</th>
                    <th>Solicitar Actualización</th>
                </tr>
            </thead>
            <tbody>
                <?php foreach ($users as $user): ?>
                    <tr id="row_<?= $user['id'] ?>">
                        <td><input type="checkbox" name="delete_user_ids[]" value="<?= $user['id'] ?>" class="delete-checkbox" style="display:none;"></td>
                        ...
                        <td><button type="submit" name="update_single" value="<?= $user['id'] ?>" onclick="return confirmUpdate();">Solicitar Actualización</button></td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
        ...
        <?php if ($_SESSION['rol'] === 'bibliotecario'): ?>
            <div class="button-group" style="display: flex; gap: 10px; margin-top: 20px;">
                ...
                <button type="submit" id="submit_button">Enviar todos los cambios</button>
